...
\end{verbatim}  
  ...
  \begin{enumerate}[label=\alph*)]
    ...
    \item Класът да допуска инициализация с функция, кооято намира следващ елемент от интервала. Следният пример ще изведе четните числа в интервала $[1..10]$:
\begin{verbatim}
...
\end{verbatim}  
  ...
  \item Да се разработи шаблон на клас \code{Repeat<T>}, с чиято помощ може да се обходи редицата $a,f(a),f(f(a)),..,f^k(a)$, $a \in T$, $f:T \rightarrow T$, такава, че за всеки елемент на редицата е е изпълнен предиката $p:T->bool$, но не е вярно, че $p(f^{k+1}(a))$. Т.е. най-дългата редица от последователни приложения на функцията $f$ над аргумент $a$, удовлетвяряващи някакво условие. Следният пример ще изведе четните числа в интервала $[0..100]$:
  \begin{verbatim}
    Repeat<int> r{0,
