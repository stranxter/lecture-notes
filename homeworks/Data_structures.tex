...
\end{verbatim}
Възможно е също да реализираме операция с интервал, която изменя броя или реда на елементите му, отново без знание за подлежащия контейнер. Такава е, например, операцията \code{filter}. Това налага да се конструира нов интервал, като по някакъв начин се измени поведението на оригиналните итератори. Например, реализирания на лекции \code{FilterRange} (наречен ``адаптор'') ``изменя'' даден итератор като ``прескача'' елементите, които не удовлетворяват даден предикат. Можем да дефинираме:
\begin{verbatim}
...
\begin{enumerate}[resume]
  ...
  \item По аналогия на дадените примери да се дефинира функция \code{combine(a,b,f)}, която създава нов интервал от стойностите на двуместната функция \code{f}, приложена върху съответните елементи на \code{a} и \code{b}. Функцията да се демомстрира като се съберат съответните елементи на два интервала.
