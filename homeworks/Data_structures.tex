...
\begin{enumerate}[resume]
\item Да се дефинират функции \code{std::string serializeTree ([подходящ тип]t)} и \code{BinTree<int> parseTree (std::string)} за сериализация и де-сериализация на двоично дърво, съдържащо числа, като се използва ``\texttt{Haskell} формат''.
