...
\begin{document}
\title[Наследяване]{Полиморфизъм и ООП в Haskell. Наследяване}
\frame{\titlepage}
...
\begin{itemize}
  ...
  \item Overloading (предефиниране)
...
\end{lstlisting}
  ...
  \item Overriding (имлементация на интерфейс)
\begin{lstlisting}[language=c++,basicstyle=\small]
double NewShape::area() override {return new_formula;}
\end{lstlisting}
\end{itemize}
...
\begin{frame}[fragile]
\frametitle{Ad hoc полиморфизъм vs Subclass полиморфизъм}


\begin{itemize}
  \item Предефиниране на функции (overloading vs. overriding)
  \item Кога е възможен overloading (ad hoc полиморфизъм)?
  \item Кога е възможен overriding (subclass полиморфизъм)?
\end{itemize}
...
\begin{itemize}
  \item Има ли в Haskell ``истински'' overriding?
\end{itemize}

\end{frame}

\begin{frame}[fragile]
\frametitle{data vs. type vs. newtype}
\begin{itemize}
  \item data - създава нов тип, който може да има няколко конструктора
  \item type - създава синоним на съществуващ тип. Двата типа се считат за един и същ навсякъде в програмата
  \item newtype - създава нов тип, който има точно един конструктор. По време на компилация се държи като data (типовете са различни), но по време на изпълнение се държи като type (няма runtime overhead)
\end{itemize}
\end{frame}
